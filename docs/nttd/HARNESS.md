...
```
coded_tools/nttd/config_files/   the SEEDS    survive a fresh start
coded_tools/nttd/state/         the WORKING copies + the commons
...
2. **`apps/nttd/runner.py` is at 0% coverage.** It exists and is complete — turn stamp, spend
...
game-layer tools are referenced in place; `apps/nttd/run_all.sh` brings up engine, studio and
runner and threads the session id and token; and `coded_tools/nttd/diagnose.py` computes the
