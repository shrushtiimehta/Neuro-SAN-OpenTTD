...
and it is the commonest way a run ends with a large map of track and no money. `[common]`

If you have stations without vehicles, buy vehicles. Do not build more stations. `[common]`
...
cannot be rescued by being rich. `[gameplay]`

Three components a one-year run cannot win, and should not be optimised for: `[gameplay]`
...
well under a quarter of that. Treat the rating as a scale you are near the bottom of. `[gameplay]`
...
that owes more than it owns. `[gameplay]`

### Revenue

Payment is for cargo **delivered**, and it falls the longer cargo sits in transit. So: `[common]`
...
covering it looks identical to a working one and earns nothing. `[common]`
...
hand-played air run spent 15 game days on an opening that needs 3. `[design]`
...
real aircraft ids in that era were 238 to 246. `[design]`
...
confirmed a vehicle can actually get from one end to the other. `[ns-common]`
...
next build costs, and take only the shortfall. `[common]` `[rail]`

### Patience

A new route takes time to earn. Leave running vehicles alone; do not judge a route on the step
after you built it, and never sell your whole fleet. `[common]`
