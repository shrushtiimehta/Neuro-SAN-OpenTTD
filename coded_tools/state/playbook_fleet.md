...
may fly. `[gameplay]` `[air]` `[ns-air]`

A purchase too late to return its price is worse than no purchase. `[ns-air]`
...
next step rather than the same one. `[ns-air]`
...
original's current tile. `[ns-air]` `[design]`
...
every row "not moving". Call it exactly once. `[gameplay]` `[design]`
...
and the route never starts. Never use full-load flags on a new route. `[rail]` `[design]`
...
wrong destination. `[common]` For road, one pair saturates past three or four buses, so growth is
more town pairs. `[gameplay]` For rail, one unsignalled line cannot take two trains, so growth is a
